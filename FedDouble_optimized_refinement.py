...
REFINE_CONF_THRESH_G     = 0.8     # teacher min confidence
REFINE_CONF_THRESH_L     = 0.8     # student min confidence
...
R2_RANK   = 32   # rein2 fixed rank (teacher)   (Reins 내부 정의 사용)
R3_RANK   = 32   # rein3 fixed rank (student)   (Reins 내부 정의 사용)
# ======================================================
...
def main(args):
    ...
    logging.info(f"[REFINE] ENABLE={ENABLE_REFINEMENT} | START={REFINE_START_EPOCH} | AGREE={REFINE_REQUIRE_AGREE} | TAU_G={REFINE_CONF_THRESH_G} | TAU_L={REFINE_CONF_THRESH_L} | TEACHER_ONLY={REFINE_USE_TEACHER_ONLY}")
    ...
    refined_label = torch.full((NUM_SAMPLES,), -1, dtype=torch.long)            # -1 means no refinement
    refined_conf  = torch.zeros((NUM_SAMPLES,), dtype=torch.float32)
    ...
    # Adapters: rein1=Dynamic, rein2/3=Fixed (match your Reins signature)
            ...
            for _ in range(args.local_ep):
                for x, y, idxs in loader:
                    ...
                    with torch.no_grad():
                        ...
                        is_clean = (agree_mask & conf_ok) | (probs >= CLEAN_THRESHOLD)

                        # ---- (Optional) Conservative label refinement ----
                        if ENABLE_REFINEMENT and epoch >= REFINE_START_EPOCH:
                            # candidate: meets refinement criteria
                            refine_cand = conf_ok.clone()
                            if REFINE_REQUIRE_AGREE:
                                refine_cand &= agree_mask
                            # decide new label
                            if REFINE_USE_TEACHER_ONLY:
                                ...
                                new_conf, new_label = avg_prob.max(1)
                            # apply thresholded, sticky update on CPU buffers
                            idxs_cpu = idxs.detach().to('cpu', non_blocking=True)
                            cand_cpu = refine_cand.detach().to('cpu', non_blocking=True)
                            nl_cpu   = new_label.detach().to('cpu', non_blocking=True)
                            nc_cpu   = new_conf.detach().to('cpu', non_blocking=True)
                            for j, ok in zip(idxs_cpu.tolist(), cand_cpu.tolist()):
                                if not ok:
                                    continue
                                jdx = int(j)  # 인덱스 변수로 분리
                                if REFINE_STICKY_BETTER:
                                    if float(nc_cpu[jdx].item()) > float(refined_conf[jdx].item() if torch.is_tensor(refined_conf[jdx]) else refined_conf[jdx]):
                                        refined_label[jdx] = int(nl_cpu[jdx].item())
                                        refined_conf[jdx]  = float(nc_cpu[jdx].item())
                                else:
                                    refined_label[jdx] = int(nl_cpu[jdx].item())
                                    refined_conf[jdx]  = float(nc_cpu[jdx].item())
                    ...
                    # gather refined labels for this batch (CPU->CUDA)
                    if ENABLE_CLEAN_CE:
                        idxs_cpu = idxs.detach().to('cpu', non_blocking=True)
                        rlab_cpu = refined_label[idxs_cpu]  # [-1 or class]
                        has_ref  = (rlab_cpu != -1)
                        if has_ref.any():
                            rlab = torch.as_tensor(rlab_cpu.numpy(), device=y.device, dtype=torch.long)
                            # for clean set, use refined label when present; else original
                            y_eff = torch.where(has_ref.to(y.device), rlab, y)
                        else:
                            y_eff = y
                    else:
