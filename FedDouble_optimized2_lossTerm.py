...
# - Train: AMP; Eval: FP32
# - Loss in Step6 (compact 3-term, each term togglable):
#   L = 1) Clean CE  +  2) Noisy KD  +  3) Adapter Prox(FedProx on adapters)
...
R2_RANK   = 32   # rein2 fixed rank (teacher)  # (주의) Reins 클래스 내부 정의 사용
R3_RANK   = 32   # rein3 fixed rank (student)  # (주의) Reins 클래스 내부 정의 사용
# ======================================================
...
def setup_logging():
    ...
    fh = logging.FileHandler('FedDouble_optimized_lossterm.txt', mode='a')
...
def main(args):
    ...
    logging.info(f"[LOSS TOGGLES] CLEAN_CE={ENABLE_CLEAN_CE} | NOISY_KD={ENABLE_NOISY_KD} | ADAPTER_PROX={ENABLE_ADAPTER_PROX}")
    ...
    # Adapters: rein1=Dynamic, rein2/3=Fixed (주의: 프로젝트의 Reins 시그니처에 맞춤)
    ...
    for epoch in tqdm(range(args.round3), desc="MainLoop-Epoch"):
        ...
        # Step6: train rein3 with agreement mask (teacher = global.reins2) — compact 3-term loss
        logging.info("Step6: Train rein3 with compact 3-term loss (CE(clean) + KD(noisy) + AdapterProx)")
        global_model.eval()
        # cache teacher adapter params for prox
        ...
        for cid in tqdm(range(args.num_clients), desc="Step6 Clients", leave=False):
            ...
            for _ in range(args.local_ep):
                for x, y, idxs in loader:
                    ...
                    # agreement + hysteresis → clean mask
                    with torch.no_grad():
                    ...
                    # --- loss (3-term compact; togglable) ---
                    l_logit = l_logit.float()
                    loss = 0.0

                    # (1) Clean CE
                    if ENABLE_CLEAN_CE and is_clean.any():
                        loss = loss + F.cross_entropy(l_logit[is_clean], y[is_clean])

                    # (2) Noisy KD (KL with temperature)
                    nz = ~is_clean
